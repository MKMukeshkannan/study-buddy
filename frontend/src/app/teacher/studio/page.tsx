...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(BACKEND + "api/v1/teacher/generate-canvas", {
        method: "POST",
      ...
      set_pages(data.canvasContent); // update state with backend response
    } catch (err) {
        ...
        <aside className="h-full bg-accent-black w-full max-w-[250px] p-10 overflow-scroll drop-shadow-xs text-white">
            ...
            <div className="space-y-2 mt-6">
              ...
              <div className="p-4 ">
                ...
                {response && (
                  <p className="mt-4 text-green-600">Server says: {response}</p>
                )}
             </div>
